...
export const useCryptoMarketsPaginated = (
  page: number = 1,
  ...
  return useQuery<
    { data: Cryptocurrency[]; totalCount: number; hasMore: boolean },
    Error
  >(
    ...
    async () => {
      ...
      let filtered = data.filter((coin: Cryptocurrency) => {
        const matchesSearch =
          coin.name.toLowerCase().includes(filters.searchQuery.toLowerCase()) ||
          coin.symbol.toLowerCase().includes(filters.searchQuery.toLowerCase());
        const matchesMarketCap =
          coin.market_cap >= filters.minMarketCap &&
          coin.market_cap <= filters.maxMarketCap;
        const matchesPrice =
          coin.current_price >= filters.minPrice &&
...
export const useCryptoMarkets = () => {
  const { filters, sortBy, sortOrder, itemsPerPage } = useAppSelector(
    (state) => state.crypto
  );
...
export const useCryptoDetail = (id: string) => {
  return useQuery<CryptoDetailData, Error>(
    ["cryptoDetail", id],
